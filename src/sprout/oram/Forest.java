...
public class Forest {
	...
	@SuppressWarnings("unchecked")
	public Forest(int unused, String filename) throws Exception {
		if (!ForestMetadata.getStatus())
			throw new ForestException("ForestMetadata is not setup");
	...
	// for testing
	public void printToFile(String filename) throws BucketException,
			TreeException, IOException, TupleException {
		...
		for (int i = 1; i < trees.size(); i++) {
			...
			for (long j = 0; j < ForestMetadata.getNumBuckets(i); j++) {
				...
				for (int k = 0; k < w; k++) {
					FileUtils.writeStringToFile(file, bucket.getTuple(k)
							.toString() + "\n", true);
				}
			}
		}
	...
	// for testing
	public void printDecryptionToFile(String filename) throws BucketException,
			TreeException, NoSuchAlgorithmException, IOException,
			TupleException {
		File file = new File(filename);
		...
		for (int i = 0; i < trees.size(); i++) {
			...
			for (long j = 0; j < ForestMetadata.getNumBuckets(i); j++) {
				Bucket bucket = t.getBucket(j);
				ECPoint x = NISTNamedCurves.getByName("P-224").getCurve()
						.decodePoint(bucket.getNonce());
				ECPoint v = x.multiply(k);
				PRG G = new PRG(bucketTupleBits); // TODO: why only fresh PRG
													// works??
				BigInteger mask = new BigInteger(G.generateBitString(
						bucketTupleBits, v), 2);
				BigInteger ptext = new BigInteger(1, bucket.getByteTuples())
						.xor(mask);
				bucket.setBucket(new byte[0],
						Util.rmSignBit(ptext.toByteArray()));
				if (i == 0)
					FileUtils
							.writeStringToFile(file, bucket.getTuple(0) + "\n");
				else
					for (int o = 0; o < w; o++) {
						FileUtils.writeStringToFile(file, bucket.getTuple(o)
								.toString() + "\n", true);
					}
			}
		}
