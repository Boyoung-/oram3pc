...
public class Tuple {
	...
	public void setTuple(byte[] tuple) throws TupleException {
		...
		if (tuple.length > tupleBytes)
			throw new TupleException("Tuple length error: " + tuple.length
					+ " != " + tupleBytes);
		else {
			this.tuple = new byte[tupleBytes];
			System.arraycopy(tuple, 0, this.tuple, tupleBytes - tuple.length,
					tuple.length);
		}
	}
	...
	public byte[] getN() {
		...
		BigInteger t = new BigInteger(1, tuple);
		return Util.rmSignBit(Util.getSubBits(t, tupleBits - 1 - nBits,
				tupleBits - 1).toByteArray());
	}
	...
	public byte[] getL() {
		...
		BigInteger t = new BigInteger(1, tuple);
		return Util.rmSignBit(Util.getSubBits(t, aBits, aBits + lBits)
				.toByteArray());
	}
	...
	public String toString() {
		...
		if (treeIndex > 0) {
			...
			builder.append("N(2)="
					+ Util.addZero(new BigInteger(1, getN()).toString(2),
							ForestMetadata.getNBits(treeIndex)) + ", ");

			builder.append("L(2)="
					+ Util.addZero(new BigInteger(1, getL()).toString(2),
							ForestMetadata.getLBits(treeIndex)) + ", ");
		}

		builder.append("A(2)="
				+ Util.addZero(new BigInteger(1, getA()).toString(2),
						ForestMetadata.getABits(treeIndex)));
