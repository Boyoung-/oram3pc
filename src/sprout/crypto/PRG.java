...
public class PRG {
	...
	public byte[] compute(byte[] seed) {
		...
		if (seed.length > 16) {
			...
			input = new byte[16];
			System.arraycopy(seed, 0, input, input.length - seed.length,
					seed.length);
		}
	...
	/*
	 * public byte[] compute(byte[] seed) { byte[] output = null; if (l <= 160)
	 * { output = leq160(seed, 0, l); } else { output = greater160(seed); }
	 * 
	 * return output; }
	 * 
	 * private byte[] leq160(byte[] input, int index, int np) {
	 * digest.update(input); if (index > 0)
	 * digest.update(BigInteger.valueOf(index).toByteArray()); byte[] ctext =
	 * digest.digest(); if (np == 160) return ctext;
	 * 
	 * byte[] output = Util.getSubBits(new BigInteger(1, ctext), 0, np)
	 * .toByteArray();
	 * 
	 * int outBytes = (np + 7) / 8; if (output.length > outBytes) output =
	 * Util.rmSignBit(output); else if (output.length < outBytes) { byte[] tmp =
	 * new byte[outBytes]; System.arraycopy(output, 0, tmp, outBytes -
	 * output.length, output.length); output = tmp; }
	 * 
	 * return output; }
	 * 
	 * private byte[] greater160(byte[] input) { int n = l / 160; byte[] front =
	 * new byte[n * 20]; for (int i = 0; i < n; i++) { byte[] seg =
	 * leq160(input, i + 1, 160); System.arraycopy(seg, 0, front, i *
	 * seg.length, seg.length); }
	 * 
	 * int np = l % 160; if (np == 0) return front;
	 * 
	 * byte[] back = leq160(input, n + 1, np); byte[] output = new BigInteger(1,
	 * front).shiftLeft(np) .or(new BigInteger(1, back)).toByteArray();
	 * 
	 * int outBytes = (l + 7) / 8; if (output.length > outBytes) output =
	 * Util.rmSignBit(output); else if (output.length < outBytes) { byte[] tmp =
	 * new byte[outBytes]; System.arraycopy(output, 0, tmp, outBytes -
	 * output.length, output.length); output = tmp; }
	 * 
	...
	/*
	 * public String generateBitString(int bits, byte[] seed) { return
	 * Util.addZero(new BigInteger(1, compute(seed)).toString(2), bits); }
	 * 
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < n; i++) {
			...
			System.out.println("deterministic:\t"
					+ (new BigInteger(1, tmp).compareTo(new BigInteger(1,
							output[i])) == 0));
			System.out.println("right length:\t"
					+ (output[i].length == outBytes));
		}
	}
}
