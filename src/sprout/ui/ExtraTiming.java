...
public class ExtraTiming {
	static BigInteger p = BigInteger.valueOf((long) Math.pow(2, 34) - 41L); // p
																			// =
																			// 2^34
																			// -
																			// 41
	...
	public static void main(String[] args) throws Exception {
		...
		for (int n = 100; n <= 1000; n += 100) {
			...
			/*
			 * for (int i=0; i<iteration; i++) { if (i == iteration/2)
			 * prg_string_sw.start(); prg.generateBitString(128*n, seed[i]); }
			 * prg_string_sw.stop();
			 */
			...
			/*
			 * aes_sw.divide(iteration); prg_string_sw.divide(iteration);
			 * prg_bytes_sw.divide(iteration); pet_sw.divide(iteration);
			 */

			System.out.print(n + ",WC(ms)");
			System.out.print("," + aes_sw.elapsedWallClockTime / convert);
			System.out
					.print("," + prg_string_sw.elapsedWallClockTime / convert);
